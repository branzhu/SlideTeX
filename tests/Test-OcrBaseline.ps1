...
function Normalize-LatexForComparison {
    ...
    $normalized = $normalized -replace '\\limits', ''
    $normalized = $normalized -replace '\\nolimits', ''
    $normalized = $normalized -replace '\\displaystyle', ''
    $normalized = $normalized -replace '\\textstyle', ''
    $normalized = $normalized -replace '\\protect', ''
    $normalized = $normalized -replace '\\bigl?', ''
    $normalized = $normalized -replace '\\bigr?', ''
    $normalized = $normalized -replace '\\Bigl?', ''
    $normalized = $normalized -replace '\\Bigr?', ''
    $normalized = $normalized -replace '\\biggl?', ''
    $normalized = $normalized -replace '\\biggr?', ''
    $normalized = $normalized -replace '\\lVert', '|'
    $normalized = $normalized -replace '\\rVert', '|'
    $normalized = $normalized -replace '\\Vert', '|'
    $normalized = $normalized -replace '\\geq', '\ge'
    $normalized = $normalized -replace '\\leq', '\le'
    $normalized = $normalized -replace '\\bm\s*', '\boldsymbol'
    $normalized = $normalized -replace '\\mathop\s*\{([^{}]*)\}', '$1'
    $normalized = $normalized -replace '\\mathrm\s*\{([^{}]*)\}', '$1'
    $normalized = $normalized -replace '\\text\s*\{([^{}]*)\}', '$1'
    $normalized = $normalized -replace '\\operatorname\s*\{([^{}]*)\}', '$1'
    $normalized = $normalized -replace '\\color\s*\{[^{}]*\}', ''
    $normalized = $normalized -replace '\\begin\s*\{[pbBvV]matrix\}', '\begin{matrix}'
    $normalized = $normalized -replace '\\end\s*\{[pbBvV]matrix\}', '\end{matrix}'
    $normalized = $normalized -replace '\\tag\*?\s*\{([^{}]*)\}', '($1)'
    $normalized = $normalized -replace '\\notag|\\nonumber', ''
    $normalized = $normalized -replace '\\begin\s*\{array\}\s*\{[^{}]*\}', ''
    $normalized = $normalized -replace '\\begin\s*\{(?:equation\*?|align\*?|gather\*?|aligned|split|cases|array|matrix)\}', ''
    $normalized = $normalized -replace '\\end\s*\{(?:equation\*?|align\*?|gather\*?|aligned|split|cases|array|matrix)\}', ''
    $normalized = $normalized -replace '&', ''
